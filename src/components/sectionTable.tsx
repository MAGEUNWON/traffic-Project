      ...
      onClick: () => {
        ...
        axios.get(`http://127.0.0.1:5000/dot`).then((response: any) => {
          ...
          response.data.forEach((e: any) => {
            // console.log(e);
            line.push({
              path: [new window.kakao.maps.LatLng(e.node_Ycode, e.node_Xcode)],
            });
          });
          ...
          for (let i = 0; i < line.length; i++) {
            ...
            polyline.push(
              new window.kakao.maps.Polyline({
                map: kakaoMap, //지도에 선을 표시
                path: item.path, // 선을 구성하는 좌표배열
                strokeWeight: 10, // 선의 두께
                strokeColor: item.color, // 선의 색깔
            ...
            window.kakao.maps.event.addListener(
              ...
              async function (e: any) {
                ...
                let coord = new window.kakao.maps.LatLng(latlng.Ma, latlng.La);

                let callback = function (result: object[], status: string) {
                ...
                if (markerArr.length === 0) {
                  ...
                  getData.data.trafficData.forEach((el: any, i: any) => {
                    if (el.linkLength !== "정보없음") {
                      saveTraffic.linkLength += el.linkLength;
                      saveTraffic.speed += el.speed;
                      saveTraffic.travelT += el.travelT;
                    }
                  });
                  ...
                  path.map((el: any, i: number) => {
                    let color = "green";
                    if (getData.data.trafficData[i]?.congestion === 1) {
                      color = "green";
                    } else if (getData.data.trafficData[i]?.congestion === 2) {
                      color = "Orange";
                    } else if (getData.data.trafficData[i]?.congestion === 3) {
                      color = "red";
                    }
                  ...
                  pline.map((el: any, i: any) => pline[i].setMap(null));
                  pline = [];
                  ...
                  getData.data.trafficData.forEach((el: any) => {
                    if (el.linkLength !== "정보없음") {
                      saveTraffic.linkLength += el.linkLength;
                      saveTraffic.speed += el.speed;
                      saveTraffic.travelT += el.travelT;
                  ...
                  path.map((el: any, index: number) => {
                    let color = "black";
                    if (getData.data.trafficData[index]?.congestion === 1) {
                      color = "green";
                    } else if (
                      getData.data.trafficData[index]?.congestion === 2
                    ) {
                      color = "Orange";
                    } else if (
                      getData.data.trafficData[index]?.congestion === 3
                    ) {
                      color = "red";
                    }
                    pline.push(
                ...
                markerObject.forEach((el, i) => {
                  window.kakao.maps.event.addListener(el, "click", (e: any) => {
                    new window.kakao.maps.InfoWindow({
                      map: kakaoMap, // 인포윈도우가 표시될 지도
                      position: new window.kakao.maps.LatLng(
            ...
            {["start", "end"].map((el, i) => {
              return (
                <div key={i}>
                  <Search
                    address={address[i]}
                    placeholder={el === "start" ? "출발지 검색" : "도착지 검색"}
                  ></Search>
                </div>
              );
            })}
